...
passer_stats <- passes_2h %>%
  filter(
    passes >= min_passes &
      next_passes >= min_passes
  )

summary(lm(next_cpox ~ cpox, passer_stats))
# coef: 0.514
# r.sq: 0.421

# pass reception in-season correlation
passes_1h <- passes_all %>%
  filter(game_index <= .5 * max(games$game_index)) %>%
  group_by(player_2) %>%
  summarize(
...
passes_2h <- passes_all %>%
  ...
  ungroup() %>%
  left_join(passes_1h, by = "player_2")

# filter to players with significant amount of data points (arbitrary selection)
min_passes_rec <- min(c(.25 * max(passes_1h$passes),
                    .25 * max(passes_2h$next_passes)))

passes_rec <- passes_2h %>%
  filter(
    passes >= min_passes_rec &
      next_passes >= min_passes_rec
  )

summary(lm(next_cpox ~ cpox, passes_rec))
# coef: 0.704
# r.sq: 0.673

r.1 <- paste0("r^2==",round(summary(lm(next_cpox ~ cpox, passer_stats))$r.sq,3))
c.1 <- paste0("coef==",round(summary(lm(next_cpox ~ cpox, passer_stats))$coef[2,1],2))
p1 <- ggplot(passer_stats, aes(cpox, next_cpox)) +
  geom_smooth(method = lm, fill = "#99D9D9", color = "#001628") +
  geom_point(alpha = .8, size = 4, color = "#001628") +
  annotate("text", x = -0.05, y = 0.08, vjust = 0, color = "red", label = r.1, parse = TRUE) +
  annotate("text", x = -0.05, y = 0.075, vjust = 1, color = "red", label = c.1, parse = TRUE) +
  theme_bw() +
  scale_x_continuous(limits = c(-.111, .111), breaks = scales::pretty_breaks()) +
  scale_y_continuous(limits = c(-.11,.11), breaks = scales::pretty_breaks()) +
  labs(x = "1st half CPOx", y = "2nd half CPOx",
       subtitle = "Passer CPOx")

r.2 <- paste0("r^2==",round(summary(lm(next_cpox ~ cpox, passes_rec))$r.sq,3))
c.2 <- paste0("coef==",round(summary(lm(next_cpox ~ cpox, passes_rec))$coef[2,1],2))
p2 <- ggplot(passes_rec, aes(cpox, next_cpox)) +
  geom_smooth(method = lm, fill = "#99D9D9", color = "#001628") +
  geom_point(alpha = .8, size = 4, color = "#001628") +
  annotate("text", x = -0.05, y = 0.08, vjust = 0, color = "red", label = r.2, parse = TRUE) +
  annotate("text", x = -0.05, y = 0.075, vjust = 1, color = "red", label = c.2, parse = TRUE) +
  theme_bw() +
  scale_x_continuous(limits = c(-.111, .111), breaks = scales::pretty_breaks()) +
  scale_y_continuous(limits = c(-.11,.11), breaks = scales::pretty_breaks(), position = "right") +
  labs(x = "1st half CPOx", y = "2nd half CPOx",
       subtitle = "Pass Receiver CPOx")

p1 + p2 +
  plot_annotation(
    title = "Receiving passes is more predictive than delivering passes",
