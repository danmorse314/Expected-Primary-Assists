...
# read ohl positions taken from ohl website
ohl_positions <- read_csv("https://raw.githubusercontent.com/danmorse314/bdc21/main/ohl%20positions.csv")
...
passers <- bdc_data %>%
  left_join(
    # passer position
    ohl_positions %>% rename(passer_position = position),
    by = "player"
  ) %>%
  left_join(
    # pass receiver position
    ohl_positions %>% rename(receiver_position = position),
    by = c("player_2" = "player")
  ) %>%
  ...
  mutate(
    ...
    d_passer = ifelse(
      passer_position == "D", 1, 0
    ),
    d_receiver = ifelse(
      receiver_position == "D", 1, 0
    )
  )
...
pass_data <- passers %>%
  select(
    strength, score_differential:distance, breakout:d_receiver, complete_pass
  )

pass_model_feats <- names(pass_data %>% select(-complete_pass))
...
bdc_data %>%
  ...
  summarize(
    n = n(),
    xcp = mean(completion_probability)
  )
