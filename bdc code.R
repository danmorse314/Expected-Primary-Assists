...
pass_data <- passers %>%
  select(
    strength, score_differential, direct:d_receiver, complete_pass
  )
...
ggplot(passer_stats, aes(cpox, next_cpox)) +
  ...
  annotate("text", x = -0.05, y = 0.04, hjust = 0, vjust = 0, color = "red", label = r.1, parse = TRUE) +
  annotate("text", x = -0.05, y = 0.035, hjust = 0, vjust = 1, color = "red", label = c.1, parse = TRUE) +
  theme_bw() +
...
bdc_data %>%
  ...
  mutate(high_danger = ifelse(slot == 1 | ozone_behind_net == 1, 1, 0)) %>%
  group_by(high_danger) %>%
  summarize(
    n = n(),
...
summary(lm(assists_primary ~ xpa, data = box))
summary(lm(assists_primary ~ xpa, data = box_ev))
